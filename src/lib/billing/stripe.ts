 ...
 *
 * When STRIPE_SECRET_KEY is set, uses real Stripe APIs.
 * When unset, all Stripe calls fall back gracefully (mock / console).
 */
...
export const PLAN_LIMITS = {
  starter: {
    maxAgents: 5,
    maxMarkets: 1,
    maxInteractionsPerMonth: 10_000,
    channels: ["whatsapp", "web"],
  },
  business: {
    maxAgents: 25,
    maxMarkets: 5,
    maxInteractionsPerMonth: 100_000,
    channels: ["whatsapp", "web", "mobile", "ussd", "api"],
  },
  enterprise: {
    maxAgents: Infinity,
    maxMarkets: 13,
...
function mapDbPlanToKey(dbPlan: string): PlanKey {
  switch (dbPlan) {
    case "STARTER":
      return "starter";
    case "PROFESSIONAL":
...
// ─── Subscription (DB-backed, graceful fallback) ────
...
export async function getSubscription(tenantId: string): Promise<SubscriptionInfo> {
  try {
    ...
    if (!sub) return { plan: "trial", status: "trialing" };
    return {
      plan: mapDbPlanToKey(sub.plan),
      status: sub.status.toLowerCase(),
      currentPeriodStart: sub.currentPeriodStart,
      currentPeriodEnd: sub.currentPeriodEnd,
      agentLimit: sub.agentLimit,
      executionLimit: sub.executionLimit,
    };
  } catch {
...
export async function checkPlanLimit(
  tenantId: string,
  limitType: "agents" | "markets" | "interactions"
  ...
  try {
    ...
    if (limitType === "agents") {
      current = await prisma.agentDeployment.count({
        where: { tenantId, status: "active" },
      });
      max = limits.maxAgents;
    } else if (limitType === "markets") {
      current = await prisma.market.count({
      ...
      current = await prisma.agentExecution.count({
        where: { agent: { tenantId }, startedAt: { gte: startOfMonth } },
      });
      max = limits.maxInteractionsPerMonth;
    }

    return { allowed: max === Infinity || current < max, current, max };
  } catch {
    // DB unavailable — allow the operation rather than blocking production traffic
...
export async function createCheckoutSession(
  ...
  if (!stripe) {
    return { url: null, error: "Configure STRIPE_SECRET_KEY to enable billing" };
  }
  ...
  if (!priceId) {
    return { url: null, error: `No Stripe price configured for plan: ${plan}. Set STRIPE_PRICE_${plan.toUpperCase()}.` };
  }
